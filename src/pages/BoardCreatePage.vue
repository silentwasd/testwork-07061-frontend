<template>
    <div class="container px-4 px-lg-5 py-4 py-lg-5">
        <div class="mb-3">
            <h1 class="m-0">Новое объявление</h1>
        ...
        <div class="row row-cols-1 row-cols-lg-2">
            <div class="col order-1 order-lg-0">
                ...
                <button class="btn btn-success w-100" @click="send">Отправить на публикацию</button>
            </div>
...
export default {
    ...
    methods: {
        async send() {
            try {
                const response = await axios.post('http://localhost:8000/api/board/create', {
                    title: this.title,
                ...
                if (response.data.success) {
                    await this.$router.push({name: 'board.item', params: { item: response.data.data.id }});
                } else
